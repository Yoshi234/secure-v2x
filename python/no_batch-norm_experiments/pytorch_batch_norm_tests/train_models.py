...
sys.path.append("..")
from compact_cnn_pytorch_batch_norm import compact_cnn_pytorch_batch_norm
...
def train_model(subIdx, xdata, channelnum, samplelength, sf, ydata, model_type, FILE,
    ...
    extract_weights(my_net.state_dict().items(), file_handle=f"{model_type}_state_dict.npy")

#train only on the 9th subject, and save the model weigths to the 
def run_model(model_name, model_type="compact_cnn"):
    ...
    train_model(subIdx, xdata, channelnum, samplelength, sf, ydata, model_type, FILE=FILE, test_subj=test_subj)
...
if __name__ == "__main__":
    custom_batch_norm_model = "compact_cnn"
    pytorch_batch_norm_model = "compact_cnn_pytorch_batch_norm"
    run_model(model_name="pytorch_batch_norm", model_type=pytorch_batch_norm_model)
    run_model(model_name="custom_batch_norm", model_type=custom_batch_norm_model)
