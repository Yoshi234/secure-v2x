...
def normalizelayer(data):
    ...
    a_mean = data - mean
    variance = torch.sqrt(torch.mean((a_mean)**2, [0, 2, 3], True) + eps)
...
class Batchlayer(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            data, mean, variance = normalizelayer(input)
            self.running_mean.data = mean
            self.running_var.data = variance
        else:
            # running_var already has eps added to it
            # (input - self.running_mean) / torch.sqrt(self.running_var)  # alternative: slower?
            data = torch.div(input - self.running_mean,
                             torch.sqrt(self.running_var))
...
class compact_cnn(torch.nn.Module):
    ...
    def __init__(self, classes=2, channels=32, kernelLength=64, sampleLength=384):
        ...
        super(compact_cnn, self).__init__()
        ...
        self.conv = torch.nn.Conv2d(
            in_channels=1, out_channels=channels, kernel_size=(1, kernelLength))
